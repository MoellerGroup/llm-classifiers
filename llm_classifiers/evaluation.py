...
class EvaluationMetrics:
    ...
    def __str__(self) -> str:
        ...
        if self.field_level_accuracy:
            ...
            for field_name, acc in self.field_level_accuracy.items():
                result.append(f"  {field_name}: {acc:.4f}")
...
class ClassifierEvaluator(Generic[T]):
    ...
    def _check_correctness(self, ground_truth: T, prediction: T) -> bool:
        ...
        if self.exact_match:
            ...
            for key, value in truth_dict.items():
                if key in pred_dict and pred_dict[key] == value:
                    return True
            return False
    ...
    def compute_metrics(
        ...
        if results and hasattr(results[0].ground_truth, "model_dump"):
            ...
            field_names = results[0].ground_truth.model_dump().keys()

            for field_name in field_names:
                field_correct = 0
                for result in results:
                    has_ground_truth = hasattr(result.ground_truth, field_name)
                    has_prediction = hasattr(result.prediction, field_name)
                    if has_ground_truth and has_prediction:
                        truth_val = getattr(result.ground_truth, field_name)
                        pred_val = getattr(result.prediction, field_name)
                        if truth_val == pred_val:
                            field_correct += 1

                field_accuracy[field_name] = field_correct / total if total > 0 else 0
        ...
        if self.comparison_field and results:
            ...
            for cls in classes:
                ...
                fp = sum(confusion_matrix[other][cls_str]
                         for other in confusion_matrix if other != cls_str)
                fn = sum(confusion_matrix[cls_str][other]
                         for other in confusion_matrix[cls_str] if other != cls_str)
    ...
    def export_results(
        ...
        for result in results:
            item = {
                ...
                "ground_truth": (
                    result.ground_truth.model_dump()
                    if hasattr(result.ground_truth, "model_dump")
                    else str(result.ground_truth)
                ),
                "prediction": (
                    result.prediction.model_dump()
                    if hasattr(result.prediction, "model_dump")
